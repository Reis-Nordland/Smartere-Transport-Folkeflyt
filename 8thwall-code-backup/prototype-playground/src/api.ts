import { Coordinates, PoiType } from './components/types'
...
function fetchTransportTimes(payload: TransportTimePayload) {
  return requestWithKey(
    `https://folkeflyt.azure-api.net/pointofinterest/api/Directions?toLat=${payload.endPos.latitude}&toLon=${payload.endPos.longitude}&fromLon=${payload.startPos.longitude}&fromLat=${payload.startPos.latitude}&transportType=${payload.transportMode}`
  ).then((response) => response.json())
}
