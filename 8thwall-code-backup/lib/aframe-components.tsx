...
function AFrameScene({sceneHtml, imageTargets, components, systems, primitives, childrenToInsert}: IAFrameSceneProps) {
  React.useEffect(() => {
    if (imageTargets) { 
    ...
    if(childrenToInsert){
      console.log("Inserting children")
       ...
       childrenToInsert.forEach(child =>  map.appendChild(child))
    }
    // Cleanup
    return () => {
